...
class window final
{
  public:
    ...
    static inline void initialize(std::string name, const vector2<size_t> center, const vector2<size_t> size,
                                  bool useMica, bool useAcrylic)
    {
  ...
  private:
    ...
    static inline handle createWindow_(const char *name, const engine::vector2<size_t> center,
                                       const engine::vector2<size_t> size, bool useMica, bool useAcrylic)
    {
#ifdef WIN32
        WNDCLASS wc{};
        ...
        HWND hwnd = CreateWindowEx(0, name, name, WS_OVERLAPPEDWINDOW, center.x - size.x / 2, center.y - size.y / 2,
                                   size.x, size.y, NULL, NULL, NULL, NULL);
        if (!hwnd)
        {
            DWORD err = GetLastError();
